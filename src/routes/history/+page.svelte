<script lang="ts">
  ...
  onMount(async () => {
    ...
    const res = await window.cr.api.history()
    if (!res.ok) {
...
<div class="h-screen overflow-y-auto px-10 py-8">
  <h1 class="mb-6 text-2xl font-black">Watch History</h1>
  {#if phase === 'error'}
    <p class="text-sm text-white/50">{error}</p>
  {:else if phase === 'empty'}
    <p class="text-sm text-white/50">Nothing watched yet.</p>
  {:else}
    <PosterGrid {items} loading={phase === 'loading'} idPrefix="hist" />
  {/if}
</div>
